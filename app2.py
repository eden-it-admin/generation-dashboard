# --- test_app.py (Complete Code - Verified Plant Summary Aggregation) ---
...
with st.sidebar: 
    ...
    st.caption("SPV & Year options update based on selected Plant(s).")
        ...
        for i, plant_name in enumerate(selected_plants): 
            with plant_tabs[i]: 
                ...
                with col_pg2: 
                    ...
                    fig_dc_p=go.Figure(go.Indicator(mode="gauge+number",value=plant_dc_capacity,title={'text':"DC Capacity(MWp)"},gauge={'axis':{'range':[0,gauge_max_dc]},'bar':{'color':"mediumpurple"},'steps':[{'range':[0,gauge_max_dc],'color':"whitesmoke"}]})); 
                ...
                st.divider() 
                # --- Plant Summary Charts --- 
                if not plant_summary.empty:
                    st.markdown("#### Plant Summary Charts")

                    # Toggle switch for chart type
                    chart_type = st.radio("Select Chart Type:", ("Line Chart", "Bar Chart"))

                    if chart_type == "Line Chart":
                        # Line Chart for Budget and Actual Generation
                        fig_line = go.Figure()
                        fig_line.add_trace(go.Scatter(x=plant_summary['Month'], y=plant_summary['Budget Gen (MWHr)'], mode='lines+markers', name='Budget Gen (MWHr)', line=dict(color='blue')))
                        fig_line.add_trace(go.Scatter(x=plant_summary['Month'], y=plant_summary['Actual Gen (MWHr)'], mode='lines+markers', name='Actual Gen (MWHr)', line=dict(color='orange')))
                        fig_line.update_layout(title='Monthly Generation Comparison', xaxis_title='Month', yaxis_title='Generation (MWHr)', xaxis=dict(tickmode='linear'))
                        st.plotly_chart(fig_line, use_container_width=True)

                    elif chart_type == "Bar Chart":
                        # Bar Chart for Average Generation
                        fig_bar = go.Figure()
                        fig_bar.add_trace(go.Bar(x=plant_summary['Month'], y=plant_summary['Budget Gen (MWHr)'], name='Budget Gen (MWHr)', marker_color='blue'))
                        fig_bar.add_trace(go.Bar(x=plant_summary['Month'], y=plant_summary['Actual Gen (MWHr)'], name='Actual Gen (MWHr)', marker_color='orange'))
                        fig_bar.update_layout(title='Monthly Average Generation', xaxis_title='Month', yaxis_title='Generation (MWHr)', barmode='group')
                        st.plotly_chart(fig_bar, use_container_width=True)

                st.divider() 
                # --- Selected Years Summary --- 
                if selected_years: 
                    st.markdown("#### Selected Years Summary (Aggregated Across SPVs)") 
                    ...
                    for spv_name in spvs_in_filtered_data: 
                        with st.expander(f"View Details for SPV: {spv_name}", expanded=False): 
                            ...
                            with col_spv_g1: 
                                ...
                                fig_ac_s=go.Figure(go.Indicator(mode="gauge+number",value=spv_ac_cap,title={'text':"AC Capacity(MW)"},gauge={'axis':{'range':[0,gauge_max_sac]},'bar':{'color':"#FF6347"},'steps':[{'range':[0,gauge_max_sac],'color':"whitesmoke"}]})); 
                            ...
                            with col_spv_g2: 
                                ...
                                fig_dc_s=go.Figure(go.Indicator(mode="gauge+number",value=spv_dc_cap,title={'text':"DC Capacity (MWp)"},gauge={'axis':{'range':[0,gauge_max_sdc]},'bar':{'color':"teal"},'steps':[{'range':[0,gauge_max_sdc],'color':"whitesmoke"}]})); 
